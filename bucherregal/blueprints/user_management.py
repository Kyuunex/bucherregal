...
from bucherregal.reusables.user_validation import validate_user_credentials
...
def login_attempt():
    ...
    if request.method == 'POST':
        ...
        resp = make_response(redirect(url_for("bookshelf.index")))
...
def profile(user_id):
    ...
    info_db = tuple(db_cursor.execute("SELECT id, email, username, display_name, permissions, email_is_public, "
                                      ...
                                      [user_id]))
    user = User(info_db[0])
    return render_template("profile.html", WEBSITE_CONTEXT=website_context, USER=user, USER_CONTEXT=user_context)
